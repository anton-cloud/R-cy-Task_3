...
const categoryNames = ["Task", "Random Thought", "Idea"];

export const addNoteBodySchema = yup
  .object({
    name: yup.string().min(1).required("Please provide a name of note."),
    category: yup
      .string()
      .oneOf(categoryNames, "Category must be: Task, Random Thought or Idea.")
      .required("Please provide a category of note."),
    content: yup.string().min(1).required("Please provide a content of note."),
  })
  .strict()
  .noUnknown(true);
...
yup.addMethod(yup.object, "atLeastOneOf", function (list: string[]) {
  return this.test({
    name: "atLeastOneOf",
    message: `Object must have at least one of these keys: ${list.join(", ")}`,
    exclusive: true,
...
export const editNoteBodySchema = yup
  .object({
  ...
  .atLeastOneOf(["name", "category", "content", "isArchived"])
  .strict()
  .noUnknown(true);
...
export const uuidSchema = yup
  .object({
    id: yup
      .string()
      .required("Please provide a content the id of note.")
      .uuid(),
  })
  .strict()
  .noUnknown(true);
